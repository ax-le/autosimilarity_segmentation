...
import as_seg.baseline_segmenter.foote.msaf_copy as msaf_foote
import numpy as np
from scipy import signal
from scipy.ndimage import filters

def process_msaf_foote(input_spectrogram = None, input_ssm = None, M_gaussian = 66, L_peaks = 64, pre_filter = 12, post_filter = 4):
        """Main process.
        Returns
        -------
        est_idxs : np.array(N)
        ...
        G = msaf_foote.compute_gaussian_krnl(M_gaussian)
        #plt.imshow(S, interpolation="nearest", aspect="auto"); plt.show()

        # Compute the novelty curve
        nc = msaf_foote.compute_nc(S, G)
        ...
        if 0 not in est_idxs:
            est_idxs = np.concatenate(([0], est_idxs))
        if input_shape[0] - 1 not in est_idxs:
            est_idxs = np.concatenate((est_idxs, [input_shape[0] - 1]))
...
def compute_foote_ssm(input_spectrogram, pre_filter = 12):
    ...
    return msaf_foote.compute_ssm(spec)
...
def postprocess_msaf(est_idxs, est_labels):
        ...
        est_idxs, est_labels = msaf_foote.remove_empty_segments(est_idxs, est_labels)
...
def my_process_segmentation_level(est_idxs, est_labels, N, frame_times, dur):
    ...
    # Remove empty segments if needed
    est_times, est_labels = msaf_foote.remove_empty_segments(est_times, est_labels)
